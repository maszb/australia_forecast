...
Promotion Mai 2022 - Data Scientist mode Bootcamp\n\n
...
if page == pages[5]:
    ...
    Ce projet s'est concentré sur la prédiction de la variable RainTomorrow, et a permis de mettre en pratique les concepts étudiés dans le cadre de la formation Data Scientest. Afin de prédire la variable RainTomorrow, des modèles ont été entraînés et testés sur les bases des règles de l'art du Machine Learning. Pour cela, des traitements du jeu de données ont d'abord été effectués, puis des analyses statistiques et graphiques, permettant une meilleure compréhension du jeu de données en notre possession.  

    Après avoir travaillé sur l’ensemble des données du dataset, et dégagé des résultats à travers trois classifieurs différents, nous avions décidé de subdiviser le dataset par régions administratives (se rapprochant beaucoup des régions climatiques), et ré-entraîner des modèles avec des hyperparamètres plus adaptés aux jeux de données locales de ces régions, pour voir si ces modèles « adaptés », allaient être plus performants que ceux qui ont été appliqués pour l’ensemble du dataset de l’Australie au niveau national.


    """)

    st.image("images//conclusion_1.png")
    st.markdown("")
    st.markdown("")
    ...
    """)
    st.markdown("")
    st.image("images//conclusion_3.png")
    st.markdown("")
    st.markdown("")
...
if page == pages[7]:
    ...
    region  = ["New South Wales", "Western Australia", "Autralia", "South Australia", "Victoria", "NSW + Victoria"]
